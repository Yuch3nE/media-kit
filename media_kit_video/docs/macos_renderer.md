...
### 4. 注入 NSScreen ICC profile

gpu-next libmpv 后端如果不显式提供 ICC，会回落到 sRGB target，在 P3 / HDR 屏上颜色不正确。新增：

- `applyDisplayICCProfile()`：开 `icc-profile-auto=yes`，并把 `NSScreen.main?.colorSpace?.iccProfileData` 通过 `mpv_render_context_set_parameter(MPV_RENDER_PARAM_ICC_PROFILE)` 注入。
- 监听 `NSApplication.didChangeScreenParametersNotification`，多屏切换 / 分辨率变更 / 拖窗换屏时自动重新注入。

`mpv_render_context_set_parameter` 在 advanced_control 关闭时可任意线程调用；mpv 内部用 `bstrdup` 复制 ICC 数据，不要求调用方延长生命周期。
